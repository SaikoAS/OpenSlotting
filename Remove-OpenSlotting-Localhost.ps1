...
try {
    ...
    $result = Remove-OpenSlottingLocalhostShortcuts -ShortcutDirectories $directories

    Write-Host ''
    if ($result.Removed.Count -gt 0) {
        Write-Host 'OpenSlotting Localhost shortcuts removed successfully.' -ForegroundColor Green
        $result.Removed | ForEach-Object { Write-Host "Removed: $_" }
    } else {
        Write-Host 'No managed OpenSlotting Localhost shortcuts were found.'
    }
    $result.Skipped | ForEach-Object {
        Write-Warning "Not removed because the shortcut is not managed by OpenSlotting: $_"
    }
    Write-Host 'Application files, the running server, and browser-local data were not changed.'
} catch {
