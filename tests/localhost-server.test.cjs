...
test('accepts the default HTTP Host form when configured for port 80', () => {
  ...
  const check = spawnSync(
    ...
    python.prefix.concat([
      '-c',
      'import os, runpy, sys; module = runpy.run_path(sys.argv[1]); assert module["expected_loopback_hosts"](80) == frozenset({"127.0.0.1", "127.0.0.1:80"}); assert module["expected_loopback_hosts"](8765) == frozenset({"127.0.0.1:8765"}); assert module["LoopbackServer"].allow_reuse_address is (os.name != "nt")',
      scriptPath
    ]),
