...
test('workspace startup is metadata-first and heavy preparation is delegated to an offline blob worker', () => {
  ...
  const activateBody = appSource.match(/async function activateWorkspace\(id, successKey, options\) \{([\s\S]*?)\n  async function createWorkspace/);
  ...
  assert.match(workerBody[1], /sourceSchemaVersion < 10 && validatedExport\.analyzed/);
  ...
  assert.match(createBody[1], /state\.workspaceLoading = true;\s*state\.workspaceLoadCancellable = false;\s*renderWorkspaceControls\(\);\s*try/);
...
test('browser UI declares multi-file selection and bilingual source traceability', () => {
  ...
  assert.match(indexSource, /id="mapping-source-search"/);
  assert.match(indexSource, /id="import-profile-name"/);
  assert.match(indexSource, /id="import-profile-select"/);
  assert.match(indexSource, /id="import-profile-save"/);
  assert.match(indexSource, /id="import-profile-apply"/);
  assert.match(appSource, /workspaceModel\.planImportProfile\(profile, file, state\.customFields\)/);
  assert.match(appSource, /if \(!plan\.compatible\) return setImportProfileMessage\('import_profile_unresolved'/);
  assert.match(indexSource, /id="mapping-editor-open"/);
  assert.match(indexSource, /id="mapping-drawer-backdrop"/);
  assert.match(indexSource, /id="mapping-panel"[^>]*\binert\b/);
  ...
  assert.match(indexSource, /data-source-filter="all"/);
  assert.match(indexSource, /data-source-filter="errors"/);
  assert.match(indexSource, /data-source-filter="changed"/);
  ...
  assert.match(appSource, /function buildChangedSourceRows\(file\)/);
  assert.match(appSource, /function sourceTableFilteredIndexes\(file\)/);
  assert.match(appSource, /function renderFocusedSourceColumn\(file, section\)/);
  assert.match(appSource, /data-source-column-position/);
  ...
  assert.ok(indexSource.indexOf('id="quality-overview-panel"') < indexSource.indexOf('id="issues-panel"'));
  assert.ok(indexSource.indexOf('id="issues-panel"') < indexSource.indexOf('id="mapping-panel"'));
  assert.ok(indexSource.indexOf('id="mapping-panel"') < indexSource.indexOf('id="source-table-panel"'));
...
test('incremental analysis accumulator matches batch analysis', () => {
...
test('unified data-quality findings cover validation, columns, articles, and cross-source evidence', () => {
  const orders = csv.importCsv(
    'article_id;quantity;delivery_date\nSKU-1;1;not-a-date\n',
    undefined,
    { sourceFile: { id: 'quality-orders', name: 'orders.csv', label: 'orders.csv', sourceType: 'order-lines' } }
  );
  const master = csv.importCsv(
    'article_id;article_name\nSKU-1;Master name\nSKU-1;Other name\n',
    undefined,
    { sourceFile: { id: 'quality-master', name: 'master.csv', label: 'master.csv', sourceType: 'article-master' } }
  );
  const ordersCatalog = [
    { position: 0, header: 'article_id', profile: { totalRows: 1, nonEmptyCount: 1, sampleValues: [] } },
    { position: 1, header: 'quantity', profile: { totalRows: 1, nonEmptyCount: 1, numericCompatibleCount: 1, sampleValues: [] } },
    { position: 2, header: 'delivery_date', profile: { totalRows: 1, nonEmptyCount: 1, dateCompatibleCount: 0, sampleValues: [] } }
  ];
  const combined = csv.combineImportResults([
    { id: 'quality-orders', name: 'orders.csv', label: 'orders.csv', sourceType: 'order-lines', columnCatalog: ordersCatalog, result: orders },
    { id: 'quality-master', name: 'master.csv', label: 'master.csv', sourceType: 'article-master', result: master }
  ]);
  const findings = combined.dataQualityFindings;
  assert.ok(findings.some((finding) => finding.scope === 'row' && finding.code === 'invalid_date' && finding.affectedCount === 1));
  assert.ok(findings.some((finding) => finding.scope === 'column' && finding.code === 'column_type_mismatch' && finding.sourceColumn === 'delivery_date'));
  assert.ok(findings.some((finding) => finding.scope === 'article' && finding.code === 'duplicate_master_article_id'));
  assert.ok(findings.some((finding) => finding.scope === 'cross-source' && finding.code === 'master_article_without_activity'));
  assert.ok(findings.every((finding) => ['source', 'column', 'row', 'article', 'cross-source'].includes(finding.scope)));
...
test('validation findings aggregate affected rows while retaining bounded examples', () => {
  const findings = csv.adaptValidationIssues([
    ...
    { sourceFileId: 'source-1', sourceFileLabel: 'orders.csv', sourceType: 'order-lines', sourceLine: 3, field: 'quantity', code: 'quantity_must_be_positive', severity: 'error', blocking: true, message: 'invalid' },
    { sourceFileId: 'source-1', sourceFileLabel: 'orders.csv', sourceType: 'order-lines', sourceLine: 4, field: 'quantity', code: 'quantity_must_be_positive', severity: 'error', blocking: true, message: 'invalid' },
    { sourceFileId: 'source-1', sourceFileLabel: 'orders.csv', sourceType: 'order-lines', sourceLine: 5, field: 'quantity', code: 'quantity_must_be_positive', severity: 'error', blocking: true, message: 'invalid' }
  ]);
  assert.equal(findings.length, 1);
  assert.equal(findings[0].affectedCount, 4);
...
test('custom field mappings preserve source values without changing core analysis', () => {
  const result = csv.importCsvStreaming(
    ...
    { order_id: 0, article_id: 1, quantity: 2, delivery_date: 3 },
    {
      ...
      preparationRules: { 'custom-zone': [{ type: 'trim', enabled: true }, { type: 'case-normalization', mode: 'upper', enabled: true }] }
    }
  );
  assert.equal(result.blocking, false);
  assert.deepEqual(result.rows[0].custom_fields, { 'custom-zone': 'COLD' });
  assert.deepEqual(result.rows[0].prepared_fields, ['custom-zone']);
  assert.equal(csv.analyzeRows(result.rows).articles[0].article_id, 'A1');
});

test('preparation rules run in order before validation and keep original source values available', () => {
  const text = 'order_id;article_id;quantity;delivery_date\nO-1;  sku-a  ;bad;2026-09-01\n';
  const mapping = { order_id: 0, article_id: 1, quantity: 2, delivery_date: 3 };
  const baseline = csv.importCsvStreaming(text, mapping, {
    sourceFile: { id: 'source-prep-base', name: 'prep.csv', label: 'prep.csv' }
  });
  assert.equal(baseline.rows.length, 0);
  assert.equal(baseline.issues.some((issue) => issue.field === 'quantity'), true);

  const result = csv.importCsvStreaming(text, mapping, {
    sourceFile: { id: 'source-prep', name: 'prep.csv', label: 'prep.csv' },
    preparationRules: {
      article_id: [
        { type: 'trim', enabled: true },
        { type: 'case-normalization', mode: 'upper', enabled: true }
      ],
      quantity: [{ type: 'replace-text', from: 'bad', to: '2', enabled: true }]
    }
  });
  assert.equal(result.rows.length, 1);
  assert.equal(result.rows[0].article_id, 'SKU-A');
  assert.equal(result.rows[0].quantity, 20000000n);
  assert.deepEqual(result.rows[0].prepared_fields.sort(), ['article_id', 'quantity']);
  assert.deepEqual(csv.reconstructRawSource(text, 2).raw_values, ['O-1', '  sku-a  ', 'bad', '2026-09-01']);
  assert.deepEqual(csv.applyPreparationRules('  Trocken  ', [
    { type: 'trim' },
    { type: 'replace-text', from: 'Trocken', to: 'Sortiment' },
    { type: 'case-normalization', mode: 'upper' }
  ]), { value: 'SORTIMENT', applied: [0, 1, 2] });
  assert.deepEqual(csv.applyPreparationRules('unTrocken', [
    { type: 'replace-text', from: 'Trocken', to: 'Sortiment' },
  ...
  const finding = combined.dataQualityFindings.find((item) => item.code === 'preparation_rule_applied' && item.field === 'article_id');
  assert.equal(finding.severity, 'info');
  assert.equal(finding.affectedCount, 1);
});

test('sentinel preparation preserves raw and prepared values on required-field findings', () => {
  const result = csv.importCsvStreaming('order_id;article_id;quantity;delivery_date\nO-1;N/A;1;2026-09-01\n', {
    order_id: 0, article_id: 1, quantity: 2, delivery_date: 3
  }, {
    sourceFile: { id: 'source-prep-sentinel', name: 'sentinel.csv', label: 'sentinel.csv' },
    preparationRules: { article_id: [{ type: 'empty-to-null', values: ['N/A'], enabled: true }] }
  });
  assert.equal(result.rows.length, 0);
  const issue = result.issues.find((item) => item.field === 'article_id' && item.code === 'required_value_missing');
  assert.equal(issue.originalRawValue, 'N/A');
