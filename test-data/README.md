...
```text
order_id;article_id;quantity;delivery_date;customer_id;sales_value;location
```
...
| `multi-export-blocked.csv` | Blocking source in the V0.2 multi-export scenario | Missing the required delivery-date column; the file is excluded while ready files remain analyzable |
